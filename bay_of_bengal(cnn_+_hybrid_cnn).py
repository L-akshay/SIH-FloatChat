# -*- coding: utf-8 -*-
"""Bay Of Bengal(CNN + Hybrid CNN).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ohpc33ZLb59NEHYBCy3bKeHmc_DEBSSz
"""
    ...
    try:
        ...
        for profile_id, group in profile_groups:
            ...
            try:
                ...
                temp_threshold = surface_temp - 0.5  # 0.5°C temperature criterion
...
def analyze_your_argo_data():
    ...
    print(f"    Lat range: {locations[:, 0].min():.1f}° to {locations[:, 0].max():.1f}°")
    print(f"    Lon range: {locations[:, 1].min():.1f}° to {locations[:, 1].max():.1f}°")
    ...
    for name, preds in [('CNN', cnn_predictions), ('Hybrid CNN+Traditional', hybrid_predictions), ('Traditional RF', traditional_predictions)]:
        ...
        models_results[name] = {'MSE': mse, 'R²': r2, 'MAE': mae, 'predictions': preds}
        print(f"   {name:20} | R²: {r2:.4f} | MAE: {mae:.2f}m | MSE: {mse:.2f}")

    # Step 6: Create visualizations
    print(f"\n Creating visualizations...")

    # Find best model
    best_model_name = max(models_results.keys(), key=lambda x: models_results[x]['R²'])
    best_predictions = models_results[best_model_name]['predictions']

    print(f"    Best model: {best_model_name} (R² = {models_results[best_model_name]['R²']:.4f})")
    ...
    geo_map = px.scatter_mapbox(
        ...
        title=f'CNN Model Geographic Performance<br>Your Argo Data Region: [20°E-120°E, 30°S-30°N]',
    ...
    for i, idx in enumerate(example_indices):
        ...
        line1 = ax.plot(profile[:, 0], depths, 'r-', linewidth=2, label='Temperature (°C)')
    ...
    print(f" Region: Indo-Pacific Ocean [20°E-120°E, 30°S-30°N]")
    ...
    print(f"    R² Score: {models_results[best_model_name]['R²']:.4f} ({models_results[best_model_name]['R²']*100:.1f}% accuracy)")
    print(f"    Mean Error: {models_results[best_model_name]['MAE']:.2f} meters")
    print(f"    Improvement over traditional: {((models_results[best_model_name]['R²'] - models_results['Traditional RF']['R²'])/models_results['Traditional RF']['R²']*100):.1f}%")
    print(f"")
